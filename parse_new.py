...
def parse_kyiv():
    ...
    for page in range(math.ceil(int(data['count']) / 1000.0)):
        ...
        for ad_id in data['items']:
            ...
            if Advertisement.objects.filter(code=advertisement.code).count() == 0:
                advertisement.save()
                print(advertisement)
